...
public class Configuration {
	...
	Configuration(ModConfigSpec.Builder builder) {
		enableFilter = builder
			.comment("Enable deny- or allowlisting items and blocks.")
			.define("enable_filter", true);
		filterMode = builder
			.comment("Set to 0 for denylist (all blocks/items EXCEPT the ones listed will be used for randomization), 1 for allowlist (only the blocks/items listed will be used for randomization). Make sure to set enable_filter to true if you want to use the lists.")
			.defineInRange("filter_mode", 0, 0, 1);
		filteredBlocks = builder
			.comment("These blocks will be filtered if enable_filter is set to true. Whether to use allow- or denylisting is defined by the filter_mode option. Use the block's registry name. E.g. to filter grass, use minecraft:grass")
		...
		filteredItems = builder
			.comment("These items will be filtered if enable_filter is set to true. Whether to use allow- or denylisting is defined by the filter_mode option. Use the item's registry name. E.g. to filter sticks, use minecraft:stick")
			.defineList("filtered_items", Lists.newArrayList(
					"minecraft:command_block_minecart",
					"minecraft:knowledge_book"),
				e -> e instanceof String);
		filteredMods = builder
			.comment("These mods will be filtered according to filter_mode if enable_filter is set to true. This list contains mod IDs.")
